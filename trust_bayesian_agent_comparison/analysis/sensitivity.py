...
def sweep_learning_params(
    ...
    t_init_grid: Optional[np.ndarray] = None,
    ...
    if t_init_grid is None:
        t_init_grid = np.array([0.0])  # Default initial trust
    
    param_combinations = list(product(
        eta_grid, memory_discount_grid, trust_discount_grid, trust_smoothing_grid,
        loss_aversion_grid, lambda_surprise_grid, inverse_temperature_grid, initial_x_grid,
        t_init_grid, seeds
    ))
    ...
    results = Parallel(n_jobs=n_jobs, verbose=verbose)(
        delayed(run_single_param_simulation)(
            eta, md, td, ts, la, ls, it, ix, ti, seed,
            partner_factory, num_rounds, threshold_direction
        ) for eta, md, td, ts, la, ls, it, ix, ti, seed in param_combinations
    )
